...
export const useStaticData = () => {
    ...
    const getFranchiseeData = async (userId) => {
        ...
        try {
            ...
            const { data, error } = await Promise.race([
                supabase
                    ...
                    .eq('user_id', userId)
                    .maybeSingle(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Franchisee timeout')), 8000))
            ]);
            if (data && !error) {
    ...
    const getRestaurantsData = async (franchiseeId) => {
        ...
        try {
          ...
          `)
                    ...
                    .eq('status', 'active'),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Restaurants timeout')), 8000))
            ]);
