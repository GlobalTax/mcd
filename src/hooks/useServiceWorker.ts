...
export const useServiceWorker = () => {
          ...
          newWorker.addEventListener('statechange', () => {
            if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
              ...
              addNotification(createNotification.info(
                ...
                'Hay una nueva versión de la aplicación disponible. Recarga para actualizar.'
              ));
            }
          });
        }
  ...
  useEffect(() => {
    const handleBeforeInstallPrompt = (e: Event) => {
      ...
      addNotification(createNotification.info(
        'Instalar App',
        'Puedes instalar esta aplicación en tu dispositivo para un mejor acceso.'
      ));
    };
    ...
    try {
      ...
      if (permission === 'granted') {
        ...
        addNotification(createNotification.warning(
          'Permisos Denegados',
          'No podrás recibir notificaciones importantes.'
        ));
  ...
  const clearCache = useCallback(async () => {
    ...
    try {
      ...
      addNotification(createNotification.success(
        'Caché Limpiado',
        'Se han eliminado todos los datos en caché.'
      ));
