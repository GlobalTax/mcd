...
export const useBudgetExport = () => {
  const exportToCSV = useCallback((data: BudgetData[], year: number, restaurantName?: string) => {
  ...
  const exportToExcel = useCallback((data: BudgetData[], year: number, restaurantName?: string) => {
    const headers = ['Categoría', 'Subcategoría', 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic', 'Total'];
