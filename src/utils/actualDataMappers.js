...
export const getMonthNumber = (monthKey) => {
    const monthMap = {
...
export const getFieldMapping = (category, subcategory) => {
    const mappings = {
...
export const groupDataByCategories = (data) => {
    const groupedData = {};
    data.forEach(record => {
        const categories = [
        ...
        categories.forEach(cat => {
            ...
            if (monthKey) {
                const currentValue = groupedData[key][monthKey];
                groupedData[key][monthKey] = currentValue + (cat.value || 0);
            }
        });
    });
    return groupedData;
