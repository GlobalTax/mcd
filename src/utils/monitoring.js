class MonitoringService {
    ...
    async trackAsyncPerformance(name, fn) {
        if (!this.isEnabled)
            return fn();
        const startTime = performance.now();
        try {
    ...
    // Logging de errores
    logError(error, context = {}, userId, sessionId) {
        if (!this.isEnabled)
            return;
        this.errorLogs.push({
            error,
    ...
    trackUserAction(action, component, metadata, userId, sessionId) {
        if (!this.isEnabled)
            return;
    ...
    trackAppPerformance() {
        ...
        if (performance.getEntriesByType) {
            ...
            clsEntries.forEach(entry => {
                ...
                if (entry.hadRecentInput !== undefined) {
