...
export interface WorkflowStep {
  ...
  name: string;
  description: string;
  type: 'task' | 'approval' | 'notification' | 'condition' | 'action';
  assignee?: string;
...
export interface WorkflowAction {
  type: 'update_field' | 'send_notification' | 'call_api' | 'create_record' | 'custom';
  target?: string;
  value?: any;
  customFunction?: string;
  metadata?: Record<string, any>;
...
export interface WorkflowTrigger {
  type: 'on_create' | 'on_update' | 'on_delete' | 'on_field_change' | 'manual' | 'scheduled';
  resource: string;
  conditions?: WorkflowCondition[];
...
export interface WorkflowHistoryItem {
  id: string;
  stepId: string;
  action: 'started' | 'completed' | 'failed' | 'skipped' | 'approved' | 'rejected';
  timestamp: Date;
  userId?: string;
  comment?: string;
  data?: Record<string, any>;
...
class WorkflowManager {
  ...
  startWorkflow(workflowId: string, data: Record<string, any>, createdBy: string): string {
    const workflow = this.workflows.get(workflowId);
    if (!workflow) {
  ...
  private executeStep(instanceId: string, stepIndex: number): void {
  ...
  // Ejecutar acción
  private executeAction(action: WorkflowAction, instance: WorkflowInstance): void {
    switch (action.type) {
      case 'update_field':
  ...
  // Asignar paso
  private assignStep(instanceId: string, stepIndex: number, step: WorkflowStep): void {
    const instance = this.instances.get(instanceId);
    if (!instance) return;
  ...
  private evaluateConditionStep(instanceId: string, stepIndex: number, step: WorkflowStep): void {
  ...
  private addHistoryItem(instanceId: string, stepId: string, action: string, userId?: string, comment?: string, data?: Record<string, any>): void {
    const instance = this.instances.get(instanceId);
    if (!instance) return;
  ...
  // Aprobar paso
  approveStep(instanceId: string, stepId: string, userId: string, comment?: string): void {
    const instance = this.instances.get(instanceId);
    if (!instance) return;
  ...
  // Rechazar paso
  rejectStep(instanceId: string, stepId: string, userId: string, comment?: string): void {
    const instance = this.instances.get(instanceId);
    if (!instance) return;
  ...
  getInstancesByUser(userId: string): WorkflowInstance[] {
    return Array.from(this.instances.values()).filter(instance => 
      Object.values(instance.assignees).includes(userId) && instance.status === 'running'
    );
  }
  ...
  // Crear template
  createTemplate(template: Omit<WorkflowTemplate, 'id' | 'usage' | 'rating' | 'createdAt'>): string {
  ...
  private findUserByRole(role: string): string | undefined {
    // Implementar búsqueda de usuario por rol
    return undefined;
  }

  private findUserByGroup(group: string): string | undefined {
    // Implementar búsqueda de usuario por grupo
    return undefined;
  }

  private findAutoAssignee(step: WorkflowStep, data: Record<string, any>): string | undefined {
