// Sistema de Temas Dinámicos para McDonald's Portal
...
class ThemeManager {
  ...
  createCustomTheme(theme: Omit<Theme, 'id' | 'isCustom'>): string {
  ...
  private adjustLightness(hex: string, lightness: number): string {
    ...
    const newR = Math.round(rgb.r + (255 - rgb.r) * (lightness as number));
    const newG = Math.round(rgb.g + (255 - rgb.g) * (lightness as number));
    const newB = Math.round(rgb.b + (255 - rgb.b) * (lightness as number));
...
export const useTheme = () => {
  ...
  return {
    ...
    createCustomTheme: (theme: Omit<Theme, 'id' | 'isCustom'>) => themeManager.createCustomTheme(theme),
    updateCustomTheme: (updates: Partial<Theme>) => themeManager.updateCustomTheme(updates),
    ...
    generateColorPalette: (baseColor: string) => themeManager.generateColorPalette(baseColor)
  };
}; 